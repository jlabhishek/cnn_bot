...
action_val = 'Files/q__action_table.txt'
...
def start_simulation():
	...
	if file_count != 0:
		agent.model.load_weights(savePath+str(file_count)+".h5")
	...
	# save the model
	if (e+1) % 50 == 0:
		agent.model.save_weights(self.savePath+str(file_count + 1 + (e+1)/50 )+'.h5')
		agent.saveQValues(q_val)
		agent.saveQActions(action_val)
